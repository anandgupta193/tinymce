...
describe('browser.tinymce.core.dom.DOMUtils', () => {
  ...
  it('DOMUtils.loadCSS contentCssCors enabled', function () {
    let c = 0;
  ...
  it('DOMUtils.isEmpty without defined schema', () => {
    DOM.add(document.body, 'div', { id: 'test' }, '');
  ...
  it('DOMUtils.isEmpty with list of elements considered non-empty', () => {
    const elm = DOM.create('p', null, '<img>');
    assert.isFalse(DOM.isEmpty(elm, { img: true }));
  });

  it('DOMUtils.isEmpty on pre', () => {
    const elm = DOM.create('pre', null, '  ');
    assert.isFalse(DOM.isEmpty(elm));
  });

  it('DOMUtils.isEmpty with list of elements considered non-empty without schema', () => {
  ...
  it('DOMUtils.get - does not find element by name in head', () => {
    const DOM = DOMUtils(document, { keep_values: true, schema: Schema() });
  ...
  it('DOMUtils.get - does not find element by name in body', () => {
    const DOM = DOMUtils(document, { keep_values: true, schema: Schema() });
  ...
  it('DOMUtils.get - finds element by id in body, not element by name in head', () => {
    const DOM = DOMUtils(document, { keep_values: true, schema: Schema() });
  ...
  it('DOMUtils.get - returns element', () => {
    const DOM = DOMUtils(document, { keep_values: true, schema: Schema() });
